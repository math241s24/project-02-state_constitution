...
=======
>>>>>>> 8ee2473033c6a4869b9098aea2231fa2998d1ea3
## New Hampshire
...
federal_url <- "https://www.wordservice.org/State%20Constitutions/usa1025.htm"
federal_html <- read_html(federal_url)

federal_text <- html_text(federal_html)

federal_con <- data.frame(word = unlist(strsplit(va_text, "\\s+"))) %>%
  filter(word != "")  %>%
  mutate(word = str_replace_all(word, "[[:punct:][:space:]]+", "")) 

rows_to_exclude_federal <- c(1:201, 3680:3689)
